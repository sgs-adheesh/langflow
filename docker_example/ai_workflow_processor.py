...
class WorkflowProcessor:
    ...
    async def _process_components(
        ...
        # Create a mapping of common AI-generated names to actual component names
        name_mapping = {
            "PromptTemplate": "Prompt",
            "ChatInput": "Chat Input",
            "ChatOutput": "Chat Output",
            "OpenAIModel": "OpenAI",
            "LLM": "OpenAI",  # Common AI mistake
            "Model": "OpenAI",  # Common AI mistake
        }
            ...
            try:
                ...
                for category, category_components in all_types_dict.items():
                    if component_type in category_components:
                        ...
                        original_component_name = component_type
                        logger.info(f"Found exact match for component '{component_type}' in category '{category}'")
                        break
                
                # If no exact match, try name mapping
                if not component_template and component_type in name_mapping:
                    mapped_name = name_mapping[component_type]
                    logger.info(f"Trying mapped name '{mapped_name}' for component '{component_type}'")
                    for category, category_components in all_types_dict.items():
                        if mapped_name in category_components:
                            component_template = category_components[mapped_name]
                            original_component_name = mapped_name
                            logger.info(f"Found mapped match for component '{component_type}' -> '{mapped_name}' in category '{category}'")
                            break
                
                # If still no match, try fuzzy matching by display name
                if not component_template:
                    for category, category_components in all_types_dict.items():
                        for name, comp_data in category_components.items():
                            display_name = comp_data.get("display_name", "")
                            # Check if AI name is contained in actual display name or vice versa
                            if (component_type.lower().replace(" ", "") in display_name.lower().replace(" ", "") or
                                display_name.lower().replace(" ", "") in component_type.lower().replace(" ", "")):
                                component_template = comp_data
                                original_component_name = name
                                logger.info(f"Found fuzzy match for component '{component_type}' -> '{name}' (display: '{display_name}') in category '{category}'")
                                break
                ...
                if not component_template:
                    ...
                    logger.info(f"Found component template for '{component_type}' -> '{original_component_name}'")
    ...
    def _create_template(self, component: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if "ChatInput" in component_type or "Chat Input" in component_type:
            template["input_value"] = {"type": "str", "value": ""}
            template["input_key"] = {"type": "str", "value": "text"}
        elif "ChatOutput" in component_type or "Chat Output" in component_type:
            template["text"] = {"type": "str", "value": ""}
        elif "File" in component_type:
            template["path"] = {"type": "str", "value": ""}
        elif "Prompt" in component_type:
