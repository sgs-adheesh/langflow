...
from langflow.api.v1.schemas import ProcessAIWorkflowRequest, ProcessAIWorkflowResponse
...
async def process_ai_workflow_endpoint(
    ...
    try:
        ...
        await logger.adebug("Loading component templates from /api/v1/all")
        component_templates = await get_and_cache_all_types_dict(
            settings_service=get_settings_service()
        )
        await logger.adebug(f"Loaded {len(component_templates)} component categories")
